...
class DoubleLinkedList:
    ...
    def sameSame(self, otherlist):
        ...
        # To do
        index = 0
        currNode0 = self._head._next
        currNode1 = otherlist._head._next
        while index < self._size:
            if currNode0._element != currNode1._element:
                return False
            index += 1
        return True
    ...
    def feed(self, otherlist, n):
        ...
        # To do            
        index = 0
        currNode = otherlist._head._next
        memo = []
        while index < n:
            otherlist._head._next = currNode._next
            currNode._next._prev = otherlist._head
            otherlist._size -= 1
            memo.append(currNode)
            currNode = otherlist._head._next
            index += 1
            
        while memo != []:
            to_feed = memo.pop()
            to_feed._next = self._head._next
            to_feed._prev = self._head
            self._head._next._prev = to_feed
            self._head._next = to_feed
            self._size += 1
    ...
    def del_anything_occured(self, otherlist):
        ...
        """
        # To do  
        currNode0 = otherlist._head._next
        while currNode0._element != None:
            currNode1 = self._head._next
            while currNode1._element != None:
                if currNode1._element == currNode0._element:
                    currNode1._prev._next = currNode1._next
                    currNode1._next._prev = currNode1._prev
                currNode1 = currNode1._next
            currNode0 = currNode0._next
