...
class Tree:
    ...
    def has_duplicate(self):
        ...
        # Task 1
        dic = {}
        new_dic = self.record_trav(self._root,dic)
        if 2 in new_dic.values():
            return True
        return False
        
    def record_trav(self,node,dic):
        if node == None:
            return dic
        if dic.get(node._element):
            dic[node._element] += 1
        else:
            dic[node._element] = 1
        trav_left = self.record_trav(node._left,dic)
        trav_right = self.record_trav(node._right,trav_left)
        return trav_right
    ...
    def is_height_balanced(self):
        ...
        for node in self.postorder():
            if abs(self.node_height(node._left)-self.node_height(node._right))>1:
                return False
        return True
    ...
    def node_height(self,node):
        if node == None:
            return -1
        return max(self.node_height(node._left),self.node_height(node._right))+1
    ...
    def sum_of_leaves(self):
        ...
        """
        # Task 3
        return self.record_leaves(self._root,0)
    
    
    def record_leaves(self,node,count):
        if node == None:
            return count
        if self.num_children(node)==0:
            count += node._element
        sum0 = self.record_leaves(node._left,count)
        return self.record_leaves(node._right, sum0)
    ...
    def evaluate(self):
        ...
        """
        # Task 6
        return self.trav_eval(self._root)
        
    
    
    def trav_eval(self,node):
        if self.is_leaf(node):
            return node._element
        else:
            right = self.trav_eval(node._right)
            left = self.trav_eval(node._left)
            operator = node._element
            result = self.calculate(operator,left,right)
            return result
...
def is_isomorphic(tree1, tree2):
    ...
    """
    # Task 4
    dic_1 = trav_dict(tree1._root,{})
    dic_2 = trav_dict(tree2._root,{})
    key_1 = dic_1.keys()
    key_2 = dic_2.keys()
    if set(key_1)^set(key_2)!= set():
        return False
    else:
        for i in key_1:
            child1 = dic_1[i]
            child2 = dic_2[i]
            if set(child1)^set(child2) != set():
                return False
        return True
    
def show_tree(tree):
    return trav_dict(tree._root,{})
...
def children(node):
    if node._left != None:
        yield node._left._element
    if node._right != None:
        yield node._right._element

def trav_dict(node,dic):
    if node == None:
        return dic
    ...
    if is_leaf(node):
        ...
        for child in children(node):
            lst.append(child)
        dic[node._element] = lst
        left = trav_dict(node._left,dic)
        right = trav_dict(node._right,left)
        return right
...
def build_expression_tree(postfix):
    ...
    for i in post_fix:
        if not is_operator(i):
            ...
            new_Tree = Tree()
            new_Tree.add_root(i)
            if isinstance(right,Tree) and not isinstance(left,Tree):
                right._root._parent = new_Tree._root
                new_Tree._root._right = right._root
                right._root = None
                right._size = 0
                new_Tree.add_left(new_Tree._root, left)
                
                
            elif isinstance(left,Tree) and not isinstance(right,Tree):
                left._root._parent = new_Tree._root
                new_Tree._root._left = left._root
                left._root = None
                left._size = None
                new_Tree.add_right(new_Tree._root,right)
                
                
            elif isinstance(right,Tree) and isinstance(right,Tree):
                right._root._parent = new_Tree._root
                new_Tree._root._right = right._root
                right._root = None
                right._size = 0
...
def main():
    ...
    print("for debug: dic of tree 3",show_tree(tree3))
    print('for debug: dic of tree 4', show_tree(tree4))
