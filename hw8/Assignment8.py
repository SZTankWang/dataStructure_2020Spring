...
class BinarySearchTree(Tree):
    ...
    def largest_to_smallest(self):
        ...
        """
        # Task 1
        result = self.largest_smallest_helper(self._root,[])
        return result
    
    
    def largest_smallest_helper(self,node, lst):
        if node == None:
            return lst
        else:
            right = self.largest_smallest_helper(node._right,lst)
            right.append(node._element)
            left = self.largest_smallest_helper(node._left,right)
            return left
    ...
    def get_kth_largest(self, k):
        ...
        lst = self.get_kth_helper(self._root,[])
        if k > self._size:
            return lst.pop()
        elif k<1:
            return lst[0]
    ...
    def get_kth_helper(self,node,lst):
        if node == None:
            return lst
        else:
            right = self.get_kth_helper(node._right,lst)
            right.append(node)
            left = self.get_kth_helper(node._left,right)
            return left
    ...
    def LCA(self, node1, node2):
        ...
        parents_1 = self.LCA_helper(node1)
        parents_2 = self.LCA_helper(node2)
        for i in parents_1:
            if i in parents_2:
                return i
    ...
    def LCA_helper(self,node):
        parent_lst = []
        parent_lst.append(node)
        currNode = node
        while currNode != None:
            parent_lst.append(currNode)
