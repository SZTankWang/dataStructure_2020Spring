...
class Tree:
    ...
    def preorderPrint(self,node):
        ...
        Prints all the elements with pre order traversal, starting at the given node.
...
# =============================================================================
    ...
    def postorderPrint(self,node):
        ...
        Prints all the elements with post order traversal, starting at the given node.
    ...
    def inorderPrint(self,node):
        ...
        Prints all the elements with in order traversal, starting at the given node.
    ...
    def traverse(self, node):
        if self.num_children(node) == 0:
            ...
            result0 = self.traverse(node._left)
            result1 = self.traverse(node._right)
            result = max(result0, result1)
            return 1+result
        elif node._left == None and node._right != None:
            return 1+ self.traverse(node._right)
        else:
            return 1+ self.traverse(node._left)
    ...
    def height(self, node = None):
        ...
        """
        if node == None:
            height = self.traverse(self._root)
            return height
        else:
    ...
    def depth(self, node):
        ...
        currNode = node
        count = 0
        while currNode._parent != None:
            currNode = currNode._parent
            count += 1
        return count
    
    def traverse_max(self, node, value):
        if self.num_children(node) == 0:
            ...
            if node._element > value:
                result0 = self.traverse_max(node._right, node._element)
                result1 = self.traverse_max(node._left, node._element)
            else:
                result0 = self.traverse_max(node._right, value)
                result1 = self.traverse_max(node._left, value)
            return max(result0,result1)
        elif node._right == None and node._left != None:
            if node._element > value:
                result = self.traverse_max(node._left, node._element)
            else:
                result = self.traverse_max(node._left, value)
            return result
        else:
            if node._element > value:
                result = self.traverse_max(node._right, node._element)
            else:
                result = self.traverse_max(node._right, value)
            return result
    def return_max(self):
        """
        :return: maximum value stored within self tree.
        """
        return self.traverse_max(self._root, self._root._element)
    ...
    def flip_node(self, node):
        ...
        :return: nothing, modify self Tree in place.
        """
        if self.num_children(node) == 2:
            temp = node._left
    ...
    def traverse_flip(self,node):
        if self.num_children(node) == 0:
            ...
            node._left = node._right
            node._right = temp
            self.traverse_flip(node._left)
            self.traverse_flip(node._right)
        elif self.num_children(node) == 1 and node._left == None:
            self.traverse_flip(node._right)
        else:
            self.traverse_flip(node._left)
    ...
    def flip_tree(self, node = None):
        ...
        and if parameter node is omitted it flips the entire tree.

        :return: nothing, modify self Tree in place.
        """
        if node == None:
            self.traverse_flip(self._root)
        else:
            self.traverse_flip(node)
