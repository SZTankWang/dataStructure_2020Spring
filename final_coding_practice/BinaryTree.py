...
class Tree:
    ...
    def height(self, node = None):
        ...
        """
        if node == None:
            return self.height_helper(0,None,self._root)
        else:
            return self.height_helper(0,None,node)
    def height_helper(self,height,prev,node):
        if node is None:
            return height
        else:

            right = self.height_helper(height,node,node._right)
            left = self.height_helper(height,node,node._left)
            if prev != None:
    ...
    def depth(self, node):
        ...
        """
        depth = 0
        return self.depth_helper(depth,node)

    def depth_helper(self,depth,node):
        if node == self._root:
            return depth
        else:
            depth += 1
            return self.depth_helper(depth,node._parent)
    ...
    def return_max(self):
        ...
        """
        return self.return_max_helper(0,self._root)
    def return_max_helper(self,maximum,node):
        if node is None:
            return maximum
        else:
            if node._element > maximum:
                maximum = node._element
            left = self.return_max_helper(maximum,node._left)
            right = self.return_max_helper(left,node._right)
            return right
    ...
    def flip_tree(self, node = None):
        ...
        """
        if node == None:
            self.flip_tree_helper(self._root)
        else:
            self.flip_tree_helper(node)

    def flip_tree_helper(self,node):
        if node is None:
            return
        else:
            self.flip_tree_helper(node._left)
            self.flip_tree_helper(node._right)
            memo = node._left
...
def main():
    ...
    print("Height of node d: Expected: 1;   Your answer: ",t.height(d))
