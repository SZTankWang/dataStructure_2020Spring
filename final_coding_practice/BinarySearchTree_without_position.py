...
class BinarySearchTree(Tree):
    ...
    def second_minimum(self):
        ...
        while curr_Node._left is not None:
            curr_Node = curr_Node._left

        #now check the node after curr_Node
        #if has right subtree, go all the way left
        if curr_Node._right is not None:
            ...
            #find the nearest node that has curr_Node in its left sub_tree
            prev = curr_Node._parent
            while prev is not None and curr_Node == prev._right:
                curr_Node = prev
                prev = prev._parent
    ...
    def iter_range(self, start, end):
        #find start
        start_node = self.search(self._root,start)
        
        if start_node._element < start:
            start_node = self.after(start_node)
        while start_node._element <= end:
            yield start_node._element
            start_node = self.after(start_node)
    ...
    def search(self,node,value):
        if node is None:
            ...
            #if curr_Node contains value
            if node._element == value:
                return node
            #if node is too big:
            elif node._element > value:
...
def main():
    ...
    for i in range(5):
        t3.insert(i)
    print("Second element in t3 is: ", t3.second_minimum())
