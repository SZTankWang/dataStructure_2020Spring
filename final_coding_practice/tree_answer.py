...
class Tree:
    ...
    def has_duplicate(self):
        values = [x._element for x in self.nodes()]
        for each in values:
            if values.count(each) >= 2:
                return True
        return False 


    def is_height_balanced(self):
        return self.balance_helper(self._root) >= 0

    def balance_helper(self, node): 
        if node == None:
            return 0
        left = self.balance_helper(node._left)
        right = self.balance_helper(node._right)
    ...
    def sum_of_leaves(self):
        if (self.root() == None):
            ...
            return self.sum_of_leaves2(self._root)

    def sum_of_leaves2(self, p):
        if (p._left is None and p._right is None):
            return p._element
        else:
            if (p._left is not None) and (p._right is not None):
                return self.sum_of_leaves2(p._left) + self.sum_of_leaves2(p._right)
            elif (p._left is not None):
                return self.sum_of_leaves2(p._left)
            elif (p._right is not None):
                return self.sum_of_leaves2(p._right)


    def evaluate(self):
        return self.evaluate_inner(self.root())

    def evaluate_inner(self, node):
        if self.is_leaf(node):
            return float(node._element)      
        else:
            operator = node._element
            left_val = self.evaluate_inner(node._left)
            right_val = self.evaluate_inner(node._right)
            if operator == '+':
                ...
                raise ValueError("Not an arithmatic operatior: "+ op)


def is_isomorphic(tree1, tree2):
    return isomorphic_helper(tree1._root, tree2._root)

def isomorphic_helper(node1, node2):
    ...
    # case 1 (line 1) not flipped;
    # case 2 (line 2) tree flipped.
    return ((isomorphic_helper(node1._left, node2._left)) and (isomorphic_helper(node1._right, node2._right))) or \
           ((isomorphic_helper(node1._left, node2._right)) and (isomorphic_helper(node1._right, node2._left)))
...
def build_expression_tree(postfix):
    ...
    inputs = postfix.split()        
    for each in inputs:
        if each.isdigit():              # Operand case
            tree = Tree()
            tree.add_root(int(each))
            stack.append(tree)
        else:                           # Operator case, pop twice
            tree = Tree()
            tree.add_root(each)
            right_branch = stack.pop()
            left_branch = stack.pop()
            tree._attach(tree._root, left_branch, right_branch)
            stack.append(tree)
    return stack.pop()  
