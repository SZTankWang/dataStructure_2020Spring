...
class BinarySearchTree(Tree):
    ...
    def minimum(self):
        '''
        :return: smallest element stored within self BST.
        '''
        # Task 2
        currNode = self._root
    ...
    def second_minimum(self):
        '''
        :return: the second smallest element stored within self BST.
        '''
        # Task 3
        #get minimum value
        currNode = self._root
        ...
        return self.after(currNode)._element
        
        
    def is_valid(self):
        '''
        :return: True if self is a valid BST. False otherwise.
        '''
        # Task 4
        lst = []
        for i in self.inorder():
            lst.append(i._element)
        #return lst
        prev = lst[0]
    ...
    def iter_range(self, start, end):
        ...
        :param end: Int -- the ending value
        :yield: yield all elements in order, such that start <= elements <= stop.
        '''
        # Task 5
        begin = self._subtree_search(self._root, start)
        #stop = self._subtree_search(self._root, end)
        currNode = begin
        if currNode._element < start:
            currNode = self.after(currNode)
        while currNode._element <= end:
...
def main():
    ...
    node_2 = test_BST.get_node(2)
    print('the value of node_2: ', node_2._element)
    print("test get node of 1: ",test_BST.before(node_2)._element)
    print("test get node of 3: ", test_BST.after(node_2)._element)
    while test_BST._size > 0:
        test_BST.delete(test_BST._root)
    print("after deletion, the size of BST: ", test_BST._size)
    print("after deletion, the root of BST: ", test_BST._root)
    #test_BST.before()
    
    #Following tree will be used for other tasks.
